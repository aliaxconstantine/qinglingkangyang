...
def get_comment():
    # 获取所有二级url地址列表
    second_url_list = list()
    for secondary_url in second_url_list:
        all_list = []
        print(secondary_url[0])
        try:
            # 评论列表
            list_of_comments = []
            soup = request2(secondary_url[0])
            # time.sleep(3)
            ...
            if page_sum != 0:
                for count in range(1, page_sum + 1):
                    second_url = f'{secondary_url[0]}-1-{count}'
...
def worker(page_num):
    try:
        ...
        if page_num > 1:
            url += f'-1-{page_num}'
        time.sleep(1)
        soup, total_pages = request(url)

        one_page_scenic_spot_info_list = scenic_spot_info(soup)
        print(one_page_scenic_spot_info_list)
        with lock:
            scenic_spot_all_info_list.extend(one_page_scenic_spot_info_list)
...
def main():
    ...
    try:
        # 启动2个线程进行爬取
        for page_num in range(1, total_pages + 1, 2):  # 每次增加5个页面
            time.sleep(1)
            for i in range(2):
                if page_num + i <= int(total_pages):
                    thread = threading.Thread(target=worker, args=(page_num + i,))
                    thread.start()
